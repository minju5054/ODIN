...
ODIN-RESCUE는 ROS 2 Humble과 Gazebo Classic 11 기반의 멀티로봇 전술 인질 구출 시뮬레이션 프로젝트입니다.
...
- 이후 단계에서는 ArUco ID `0`을 hostage surrogate로 사용해 인질 인식 이벤트를 발행합니다.
...
- `worlds/odin_rescue_20x20_c.world`: 기본 전술 맵 C입니다. 맵 B 기반에 오른쪽 위 적 요충지 오브젝트가 추가되어 있습니다.
- `worlds/odin_rescue_20x20_b.world`: C의 기반이 된 비대칭 landmark 테스트용 맵 B입니다.
...
- `odin_detection`: ArUco 기반 hostage marker 감지 및 `/hostage_events` 발행
...
- ArUco hostage detection, coordinator, robot_3 dispatch, AI 연동은 이후 milestone입니다.
