...
위 명령은 Gazebo, scout SLAM, `/merged_map`, scout 자율주행, Gazebo 기반 ArUco event detector, coordinator를 함께 실행합니다.
...
- coordinator는 후보 경로와 검증된 goal만 생성하며, 현재 milestone에서는 robot_3를 실제 dispatch하지 않습니다.
- AI 연동은 `/ai/waypoint_recommendation` 입력과 `/coordinator/validated_waypoint` 출력 topic만 준비되어 있습니다.
